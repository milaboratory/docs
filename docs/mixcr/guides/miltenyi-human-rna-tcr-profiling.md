...
??? tip "Use [aria2c](https://aria2.github.io) for efficient download of the full dataset with the proper filenames:"
    ...
    --8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/010-download-aria2c.sh"
    ```
    ```shell title="download-list.txt"
    --8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/download-list.txt"
    ```
## Upstream analysis
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/020-upstream-preset.sh"
```
...
??? tip "See first 100 records from FebControl1.clones.IGH.tsv clonotype table"
    {{ read_csv('docs/mixcr/guides/miltenyi-human-rna-tcr-profiling/figs/Multi_TRA_FS115_1.clones.tsv', engine='python', sep='\t', nrows=100) }}

In order to run the analysis for all samples in the project on Linux we can use [GNU Parallel](https://www.gnu.org/software/parallel/) in the following way:

```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/020-upstream-preset-parallel.sh"
```

### Under the hood pipeline

Under the hood `miltenyi-human-rna-tcr-profiling` preset actually executes the following pipeline:

#### `align`
[Alignment](../reference/mixcr-align.md) of raw sequencing reads against reference database of V-, D-, J- and C- gene segments.

```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/040-upstream-align.sh"
```
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/045-upstream-refineTagsAndSort.sh"
```
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/050-upstream-assemble.sh"
```
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/060-upstream-exportClones.sh"
```
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/080-qc-align.sh"
```

![align QC](miltenyi-human-rna-tcr-profiling/figs/alignQc.svg)
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/120-qc-chainUsage.sh"
```

![chain usage QC](miltenyi-human-rna-tcr-profiling/figs/chainUsage.svg)
...
```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/125-qc-exportReports.sh"
```

```shell
--8<-- "guides/miltenyi-human-rna-tcr-profiling/scripts/125-qc-exportReports-json.sh"
```

??? "Show report file"
    === "`.txt`"
        ```shell
        --8<-- "guides/miltenyi-human-rna-tcr-profiling/figs/Multi_TRA_FS115_1.report.txt"
        ```
    === "`.json`"
        ```js
        --8<-- "guides/miltenyi-human-rna-tcr-profiling/figs/Multi_TRA_FS115_1.report.json"
        ```